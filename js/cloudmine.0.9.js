...
(function() {
  ...
  WebService.prototype = {
    ...
    destroy: function(keys, options) {
      options = opts(this, options);
      if (keys == null && options.all === true) keys = {
        all: true
      };
      else keys = {
        keys: (isArray(keys) ? keys.join(',') : keys)
    ...
    search: function(query, options) {
      options = opts(this, options);
      query = {
        q: query != null ? buildSearchQuery(query) : ''
      }
      return new APICall({
        action: 'search',
        type: 'GET',
    ...
    searchUsers: function(query, options) {
      query = {
        p: query != null ? buildSearchQuery(query) : ''
      }
      options = opts(this, options);
      return new APICall({
  ...
  function APICall(config) {
    ...
    var root = '/',
      session = opts.session_token,
      applevel = opts.applevel;
    ...
    this.config.complete = function(xhr, status) {
      ...
      if (status == 'success' && self.status >= 200 && self.status < 300) {
        ...
        if (isString(self.data)) {
          self.data = {
            errors: [self.data]
          } // This way, we can rely on this structure data[statuscode].errors to always be an Array of one or more errors
        }
  ...
  APICall.binaryUpload = function(apicall, data, filename, contentType) {
    ...
    return apicall.setData(['--' + boundary, 'Content-Disposition: form-data; name="file"; filename="' + filename + '"', 'Content-Type: ' + contentType, 'Content-Transfer-Encoding: base64', '', data, '--' + boundary + '--'].join('\r\n'));
  }
  ...
  var valid_params = {
    ...
    skip: 'skip',
    snippet: 'f',
    // Run code snippet on the data
    params: 'params',
    // Only applies to code snippets, parameters for the code snippet (JSON).
    dontwait: 'async',
    // Only applies to code snippets, don't wait for results.
    resultsonly: 'result_only',
    // Only applies to code snippets, only show results from code snippet.
    count: 'count',
    distance: 'distance'
  };
  ...
  function stringify(map, sep, eol, ignore) {
    sep = sep || '=';
    var out = [],
      val, escape = ignore ? nop : encodeURIComponent;
    for (var k in map) {
  ...
  function unstringify(input, sep, eol, ignore) {
    ...
    var out = {},
      unescape = ignore ? nop : decodeURIComponent;
